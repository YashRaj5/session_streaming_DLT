...
dbutils.widgets.text("root", "dbfs:/tmp/streaming_session")
...
# MAGIC <img width="1px" src="https://www.google-analytics.com/collect?v=1&gtm=GTM-NKQ8TT7&tid=UA-163989034-1&aip=1&t=event&ec=dbdemos&ea=VIEW&dp=%2F_dbdemos%2Fdata-engineering%2Fstreaming-sessionization%2F01-Delta-session-BRONZE&cid=356223210893384&uid=2795718610587230">
# MAGIC
...
# MAGIC
# MAGIC Start the [00_session_producer]($./00_session_producer) notebook to send messages to your kafka queue. 

# COMMAND ----------

# MAGIC %run ./00_session_producer $reset_all_data=$reset_all_data
...
# DBTITLE 1,Create the table events_raws
# MAGIC %sql
# MAGIC CREATE TABLE IF NOT EXISTS events_raw (key string, value string) 
# MAGIC TBLPROPERTIES (
# MAGIC   delta.autoOptimize.optimizeWrite = true, delta.autoOptimize.autoCompact = true);
...
# DBTITLE 1,Reading the Streaming
...
stream = (spark.readStream
    ...
    .option("kafka.sasl.jaas.config", """kafkashaded.org.apache.kafka.common.security.plain.PlainLoginModule required username="Z5NHE2DJIUBYDJ5T" password="7YSfjIjuMJSzbD1LTi+E66pGQmyCJM/j9rfeLOIy4OuzF4gSX+sEqIAzCv/GjT+7";""")
    ...
    .writeStream
        ...
        .option("checkpointLocation", dbutils.widgets.get('root') + '/checkpoints/bronze')
